...
pd_df["HighTraveller"] = np.where((pd_df['avg_site_dist_day_night'] >= commuteDist75th) & (pd_df['location_diversity_index'] >= commuteDiv75th),1,0)

#################################################################################################################

rsltSegment = pd_df.query("HighTraveller==1")

rsltSegment.describe().to_csv("Travel_segment_summary.csv")


seg_var = rsltSegment.var()
seg_var.to_csv("Travel_segment_var.csv")

# Save the correlation matrix to a CSV file
correlation_matrix = rsltSegment.corr()
correlation_matrix.to_csv('Travel_corr_matrix.csv', index=True)


##################################Non-Hightraveller statistics##########################
nonSegment = pd_df.query("HighTraveller==0")

nonSegment.describe().to_csv("Non_Travel_segment_summary.csv")


seg_var = nonSegment.var()
seg_var.to_csv("Non_Travel_segment_var.csv")


# Save the correlation matrix to a CSV file
correlation_matrix = nonSegment.corr()
correlation_matrix.to_csv('Non_Travel_corr_matrix.csv', index=True)
...
importance_df.to_csv("Travel_featureimp.csv")
